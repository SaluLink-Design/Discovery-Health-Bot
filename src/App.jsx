...
import { authiKnowledgeBase, quickPrompts } from './data/authiData';

const defaultQuery = 'What treatment benefits are available for diabetes?';
const emptyResult = {
  intent: 'general',
  condition: null,
...
export default function App() {
  ...
  const submitQuery = useCallback(async (nextQuery) => {
    setLoading(true);
    setError('');
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Authi could not reach the backend.');
      }
        ...
        <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-8 shadow-2xl shadow-cyan-950/30 backdrop-blur">
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">
              This app turns the notebook&apos;s `AuthiEngine` idea into a web experience that
            ...
            <form
              className="mt-8 rounded-[1.75rem] border border-white/10 bg-white/5 p-4"
        ...
        <section className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
          ...
          <div className="grid gap-5">
            {result.sections.map((section) => (
              <ResultCard key={section.title} title={section.title} items={section.items} />
            ))}
